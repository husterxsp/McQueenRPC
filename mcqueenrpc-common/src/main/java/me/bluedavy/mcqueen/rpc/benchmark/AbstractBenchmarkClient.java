...
public abstract class AbstractBenchmarkClient {
    ...
    public void run(String[] args) throws Exception {
        ...
        startInfo.append(" bytes,the benchmark will end at:").append(dateFormat.format(calendar.getTime()));
        ...
        // concurrents 个并发同时执行，所以这里用CyclicBarrier
        // 可以看到后面的 SimpleProcessorBenchmarkClientRunnable 的run方法，
        // barrier 是为了让所有的run同时执行
        ...
        System.out.println(" Avg TPS: " + benchmarkRequest / times.keySet().size() + " Max TPS: " + maxTPS + " Min TPS: "
                + minTPS);
